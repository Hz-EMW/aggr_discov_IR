...
## C:\windows\system32>java -jar D:\docs_seleniumhq_org\selenium-server-standalone-3.11.0.jar
...
pjl <- cbind(pjc, locate[,4])
colnames(pjl) <- c("projname", "num_pro", "location")
...
colnames(pjal) <- c("deforder", "projname", "num_pro", "airport", "location", "Country")
library(dplyr)
pj_fc <- data.frame()
...
colnames(pjal) <- c("deforder", "projname", "num_pro", "location", "Country")
...
for(i in 1:nrow(city_pairs)){
  gcIntermediate(all_pairs$long1[i], all_pairs$lat1[i], all_pairs$long2[i], all_pairs$lat2[i], col="skyblue", lwd=1)
}


caw <- merge(flights.ag, worldport_pc, by.x = "AIRPORT_TO_CODE", 
                    by.y = "code", all.x = T)
